 ...
 *
 */
public class SimplePageView extends SmoothPagedView implements View.OnLongClickListener {
    ...
    public void setDatas(SparseArray<List<ItemTable>> data) {
        datas = data;
        Log.i("EasyActivity", "simplePageView 的数据：" + datas);
    }
    ...
    private void initItem(ItemTable item, View v) {
        switch (item.type) {
            ...
            case IConstant.TYPE_APP_PRD:
                ...
                Drawable d = PagedDataModel.iconCache.get(new ComponentName(item.pkgName, item.clsName));
                if (d == null) {
                ...
                if (item.type == IConstant.TYPE_APP_PRD) {
                    TextView des = (TextView) v.findViewById(R.id.prd_des_title);
                    if (des != null)
                ...
                if (item.bgResId > 0)
                    ...
                    //add by zhouerlong update fatal
                    try {
    ...
    private void initAddItem1(ItemTable item, SimplePage p, int index) {
        switch (item.type) {
            ...
            case IConstant.TYPE_APP_PRD:
                v = mInflate.inflate(R.layout.simple_app_item_prd, null);
...
//        Log.d("SimplePageView", "---------"+childHeightSize+"");
//        Log.d("Launcher.scale", "---------"+Launcher.scale+"");
...
//            Log.d("SimplePageView", "---------"+childheightMeasureSpec+"");
        }
    }
    ...
    public boolean onLongClick(View arg0) {
		...
		/*if(PreferencesManager.getCurrentPageStyle(mContext)==1) {
            Launcher l = (Launcher) mContext;
