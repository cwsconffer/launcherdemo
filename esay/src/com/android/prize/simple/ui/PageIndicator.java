...
public class PageIndicator extends LinearLayout {
    ...
    void addMarker(int index, PageMarkerResources marker, boolean allowAnimations,View child) {
    ...
    void addMarkers(ArrayList<PageMarkerResources> markers, boolean allowAnimations) {
        for (int i = 0; i < markers.size(); ++i) {
            addMarker(Integer.MAX_VALUE, markers.get(i), allowAnimations,null);
        }
    }

    void updateMarker(int index, PageMarkerResources marker) {
    	index = index==mMarkers.size()?index-1:index;
    ...
    void setGone(int i) {
    	mMarkers.get(i).setVisibility(View.GONE);
    }
