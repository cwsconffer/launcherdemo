...
import com.android.prize.simple.activity.EasyActivity;
import com.android.prize.simple.utils.Utilities;
//add by zhouerlong 
...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
	...
	enum FLING_STATE {
		NONE, UP, DOWN,MULTI_DOWN,MULTI_UP
	};// add by zhouerlong
	//add by zhouerlong

        /*PRIZE-launcher3-zhouerlong-2015-7-23-end*/
    protected static final int PAGE_SNAP_ANIMATION_DURATION = 750;
    protected static final int SLOW_PAGE_SNAP_ANIMATION_DURATION = 1000;
	...
	//add by zhouerlong 
    protected float mLastMotionPointdist;
    protected PointF mLastFastSmoothPoint; //add zhouerlong
    protected boolean mIsCanPerformeTowFinger;
	//add by zhouerlong 
    ...
    public  final static int UNITIS = 1600;
    protected final static int TOUCH_STATE_REST = 0;//未滑动
    protected final static int TOUCH_STATE_SCROLLING = 1;//滑动
    ...
    protected int mTouchYState = TOUCH_STATE_REST;
    ...
    protected int mActivePointerId1=INVALID_POINTER;
    ...
    protected int mFlingToShowSystemUIThresholdYVelocity = 800;//modify by zhouerlong
    protected int mFlingMultiFingerYVelocity = 100;//add by zhouerlong
        /*PRIZE-launcher3-zhouerlong-2015-7-23-start*/
    protected int mFlingToShowSystemUIThresholdXVelocity = 1500;//add by zhouerlong
	//add by zhouerlong
    // Drag to delete
    protected boolean mDeferringForDelete = false;
   ...
   public int getViewportHeight() {
    ...
    public PageIndicator getPageIndicator() {//add by zel
   ...
   protected View getPageAt(int index) {
    ...
    protected void pageBeginMoving() {
     /*   if (!mIsPageMovingtrue) {*/
            mIsPageMoving = true;
//            if(((Launcher) getContext()).getworkspace().getOpenFolder() != null 
//            		&& ((Launcher) getContext()).getworkspace().getOpenFolder().getAnimationingState() != Folder.STATE_OPEN)
            onPageBeginMoving();
...
//        }
    ...
    public void scrollTo(int x, int y) {
        ...
        boolean isXBeforeFirstPage = isRtl ? (x > mMaxScrollX) : (x < 0);//第一页往左推到最后一页
        boolean isXAfterLastPage = isRtl ? (x < 0) : (x > mMaxScrollX);//最后一页网页推到第一页
        if (isXBeforeFirstPage) {
//        	LogUtils.i("zhouerlong", "isXBeforeFirstPage"+":x::::"+x);
        	if(mMotionEvent != null&&mMotionEvent.getPointerCount()<=1) {
//            	requestDisallowInterceptTouchEventByScrllLayout(false);
        	}
...
//        	LogUtils.i("zhouerlong", "isXAfterLastPage"+":x::::"+x);
//        	LogUtils.i("zhouerlong", "isXBeforeFirstPage"+":x::::"+x);
        	if(mMotionEvent != null&&mMotionEvent.getPointerCount()<=1) {
        	}
            /// M: modify to cycle sliding screen.
...
//        	requestDisallowInterceptTouchEventByScrllLayout(true);
        	mOverScrollX = x;
            mOverScrollY = 0;
//            LogUtils.i("zhouerlong", "mOverScrollX:workspce:"+mOverScrollX);
            super.scrollTo(x, 0);
        }
...
//        lp.isFullScreenPage = true;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(widthSize<EasyActivity.viewHeight) {
...
//                    Log.i("zhouerlong", "alpha:"+alpha+"i:"+i);
    ...
    public void onChildViewAdded(View parent, View child) {
        ...
        if (mPageIndicator != null && !isReordering(false)) {
            ...
            mPageIndicator.addMarker(pageIndex,
                    ...
                    mAllowPagedViewAnimations,child);
        }
...
//    	}
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        int halfScreenSizeY = getViewportHeight() / 2;
        // mOverScrollX is equal to getScrollX() when we're within the normal scroll range.
        // Otherwise it is equal to the scaled overscroll position.
        int screenCenter = mOverScrollX + halfScreenSize;
        int screenCenterY = mOverScrollY+halfScreenSizeY;

        /// M: Add for cycle sliding screen for op09.
        if (isSupportCycleSlidingScreen() && this instanceof PagedView) {
     ...
     /*   Log.i(TAG, "dispatchDraw: mScrollX = " + getScrollX() + ", screenCenter = " + screenCenter
        ...
        if (screenCenter != mLastScreenCenter || mForceScreenScrolled) {
            ...
            screenScrolled(screenCenter,screenCenterY);
...
//                        	Log.i("zhouerlong", "ForceDrawAllChildrenNextFrame:"+"["+leftScreen+","+rightScreen+"]"+"i:"+i);
    ...
    public abstract void requestDisallowInterceptTouchEventByScrllLayout(boolean disallowIntercept);
...
//        LogUtils.i(TAG, "onInterceptTouchEvent: ev = " + ev +
//                ", mScrollX = " + mScrollX + ", this = " + this);
         ...
         * scrolling there.
         */
        acquireVelocityTrackerAndAddMovement(ev); //获取速率跟踪，并且将Movement添加
         ...
         * motion.最常见的情况是：用户处于拖动的状态下，同时在移动它的手指，这时候我们需要拦截这个动作。 
         */
        final int action = ev.getAction();
        if ((action == MotionEvent.ACTION_MOVE) &&
                (mTouchState == TOUCH_STATE_SCROLLING)) {
            //如果还在拖动表示 拖动尚未完成我们要returntrue
            return true;
        }
	...
	//add by zhouerlong 
        case MotionEvent.ACTION_POINTER_DOWN:
        	...
        	this.mIsCanPerformeTowFinger = true;
        	break;
	//add by zhouerlong 
...
//                	Log.i("zhouerlong", "执行滑动操作:");
                    determineScrollingStart(ev); //执行滑动操作
...
//                                scrollBy((int) deltaX, 0);
            ...
            case MotionEvent.ACTION_DOWN: {
            	...
            	mLastFastSmoothPoint = new PointF(ev.getRawX(),ev.getRawY());//A by zel

//            	if (mCurrentPage != 0) {
            		this.requestDisallowInterceptTouchEventByScrllLayout(true);
//            	}
...
//            	mOnlyFirstExcute = false;//add by zhouerlong
            	mFlingState = FLING_STATE.NONE;
                /// M: [Performance] Scroll page in first performTraversal to improve response time.
                mLastX = mLastMotionX;
                mLastMotionY = y;
                mLastRawY =mDownRawY= ev.getRawY();
                mLastY = mLastMotionY;
                ...
                if (finishedScrolling) {
                    ...
                    if(!mScroller.isFinished()) {
                        mScroller.abortAnimation();//加此方法表示如果上次的滑动还在滑动的过程 先停止滑动
                    }
                } else {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(mTouchYState == TOUCH_STATE_SCROLLING) {
                    onTouchYUp(ev);
                }
            case MotionEvent.ACTION_CANCEL:
                resetTouchState();
                mIsCanPerformeTowFinger=false;
	//add by zhouerlong 
                break;
	...
	public void rvertTouchYState() {
		this.mTouchYState = TOUCH_STATE_REST;
	}
     ...
     */
    //add by zhouerlong
//    private boolean mOnlyFirstExcute=false;
    ...
    protected void determineScrollingStart(MotionEvent ev, float touchSlopScale) {
        ...
        if (ev.getPointerCount()>1) {
			mActivePointerId1 = ev.getPointerId(1);
	        pointerIndex = ev.findPointerIndex(mActivePointerId1);
		}
        if (pointerIndex == -1) {
            return;
        }
        ...
        final int xDiff = (int) Math.abs(x - mLastMotionX);//如果在x轴滑动的距离大于down下的距离 表示启动滑动状态
        final int yDiff = (int) Math.abs(y - mLastMotionY);//如果在y轴滑动的距离大于down下的距离 表示启动滑动状态
        final float  yD = y - mLastMotionY;//如果在y轴滑动的距离大于down下的距离 表示启动滑动状态

        final int touchSlop = Math.round(touchSlopScale * mTouchSlop); //读取滑动距离最小值 超出此范围表示需要滑动
        boolean xPaged = xDiff > 4; //滑动页数最小距离

	//add by zhouerlong prizeTheme add
        boolean xMoved = xDiff > touchSlop/2;//判断是否触发滑动
        boolean yMoved = yDiff > touchSlop;//判断Y轴是否触发滑动
        boolean yMoved1 = (yDiff > touchSlop/2)&&!xMoved&&yD>0;//判断Y轴是否触发滑动
//        Log.i("zhouerlong", "xMoved:"+xMoved+"  yMoved:"+yMoved+"  xPaged:"+xPaged+"  xDiff:"+xDiff+" touchSlop:"+touchSlop);
	//add by zhouerlong prizeTheme add
        if (xMoved || xPaged || yMoved) {
            if (mUsePagingTouchSlop ? xPaged : xMoved) {
				...
				//add by zhouerlong
                if(isFlingingToShowSysteUI() == FLING_STATE.DOWN&&ev.getPointerCount()<=1) {

                    mTouchState = TOUCH_STATE_REST; //表示状态为滑动状态
                    //add by zhouerlong
//                    mOnlyFirstExcute = true;
                    mFlingState = FLING_STATE.DOWN;
                }else if (isFlingingToShowSysteUI() == FLING_STATE.UP&&ev.getPointerCount()<=1) {
                    mTouchState = TOUCH_STATE_REST; //表示状态为滑动状态
                	...
                	if(ev.getPointerCount()>1) {
                		...
                		if(mTouchYState!=TOUCH_STATE_SCROLLING)
                        mTouchState = TOUCH_STATE_SCROLLING; //表示状态为滑动状态
                	}
                    if(mFlingState == FLING_STATE.MULTI_DOWN) {
                    	mTouchState = TOUCH_STATE_REST;
                    }
                    if(mFlingState == FLING_STATE.MULTI_UP) {
                    /*	boolean isleft = ((Workspace)this).isLeftFrame();
                    	if(isleft)
                    	mTouchState = TOUCH_STATE_REST;*/
                    }
                }
				//add by zhouerlong
                mTotalMotionX += Math.abs(mLastMotionX - x); //滑动距离值
                if(ev.getPointerCount()>1) {
                	mDownMotionX =x;
                }
                mLastMotionX = x;
        ...
        if ((yMoved1||mTouchYState==TOUCH_STATE_SCROLLING)) {}
    }
    
    public float mScreenYProgress = 0f;
    
    public void onTouchYMove(float yD,View v) {
    }
    
    public void onTouchYUp(MotionEvent ev) {
    ...
    protected float getScrollProgress(int screenCenter, View v, int page) {
        ...
        if(isSupportCycleSlidingScreen()) {
            if (getScrollX()>mMaxScrollX) {
            	if (page==0) {
//            		Log.i("zhouerlong", "screenCenter: bf:"+screenCenter+" getScrollX():"+getScrollX());
            		screenCenter-=getScrollForPage(this.getChildCount()-1)+getViewportWidth();
//            		Log.i("zhouerlong", "screenCenter: "+screenCenter+" getScrollX():"+getScrollX());
            	}
            }else if (getScrollX()<0) {
            	if (page==this.getChildCount()-1) {
//            		Log.i("zhouerlong", "screenCenter: bf:"+screenCenter+" getScrollX():"+getScrollX());
            		screenCenter+=getScrollForPage(this.getChildCount()-1)+getViewportWidth();
//            		Log.i("zhouerlong", "screenCenter: "+screenCenter+" getScrollX():"+getScrollX());
            	}
    ...
    protected float getScrollProgress(int screenCenter) {
        final int halfScreenSizeY = getViewportHeight()/2;
        int totalDistance = getViewportHeight();
        int delta = screenCenter -   halfScreenSizeY;

        float scrollProgress = delta / (halfScreenSizeY/4 * 1.0f);
    ...
    protected void dampedOverScroll(float amount) {
        ...
        if (amount < 0) {
            mOverScrollX = overScrollAmount;
            super.scrollTo(0, 0/*getScrollY()*/);
        } else {
            mOverScrollX = mMaxScrollX + overScrollAmount;
            super.scrollTo(mMaxScrollX, 0/*getScrollY()*/);
        }
        invalidate();
    }
     ...
     * @return
     */
    protected float  directionDistY(PointF  last,PointF dest) {
    				
    				return dest.y-last.y;
    }
	//add by zhouerlong 
    
    protected float  directionDistX(PointF  last,PointF dest) {
    				
    				return dest.x-last.x;
    }
	...
	private void printOtherPointMotionEvent(MotionEvent ev) {
		if (ev.getPointerCount() > 1) {
			int pointId = ev.getPointerId(0);
			int pointId1 = ev.getPointerId(1);
			float x = ev.getX(pointId);
			float y = ev.getY(pointId);
			float x1 = ev.getX(pointId1);
			float y1 = ev.getY(pointId1);
	...
	/**MotionEvent为Move时间时候的执行操作
	 * 里面实现了多点触控
	 * @param ev
	 */
	private void handleMaskPointUpEvent(MotionEvent ev,boolean intercept) {
		 //如果此窗台为滑动状态 TOUCH_STATE_SCROLLING
         int activePointerId = ev.getPointerId(0); //触摸点数 （与多点触摸有关系）
         if (ev.getPointerCount()>1) {
        	 if(mActivePointerId1 ==-1) {
        		 mActivePointerId1 = ev.getPointerId(1);
        	 }
         	activePointerId = mActivePointerId1;
         	mTouchState = TOUCH_STATE_REST;
         	if(mTouchYState == TOUCH_STATE_SCROLLING&& ev.getPointerCount()<=1) {
             	mTouchYState = TOUCH_STATE_REST;
         	}
         }
         int pointerIndex = ev.findPointerIndex(activePointerId); //获取当前点
         final float x;
        try{
    	    x = ev.getX(pointerIndex);//当前滑动的x坐标
       }catch(IllegalArgumentException ex) {
    	   return ;
       }
        
        final VelocityTracker velocityTracker = mVelocityTracker;
        velocityTracker.computeCurrentVelocity(UNITIS);//modify by xiaxuefeng 
        int velocityX = (int) velocityTracker.getXVelocity(activePointerId);
        int velocityY = (int) velocityTracker.getYVelocity(activePointerId);//A by zel
		//add by zhouerlong
        final int deltaX = (int) (x - mDownMotionX); //移动的距离 负数表示向右话滑动，正数表示向左滑动
        if (getPageAt(mCurrentPage) == null) {
        	return ;
        }
        final int pageWidth = getPageAt(mCurrentPage).getMeasuredWidth(); // 
        boolean isSignificantMove = Math.abs(deltaX) > pageWidth *
                SIGNIFICANT_MOVE_THRESHOLD; //是否是跳页数 isSignificantMove 
        ...
        boolean isFling = mTotalMotionX > MIN_LENGTH_FOR_FLING &&
				...
				//A by zel
        boolean isVelocityY =  Math.abs(velocityY) > 0;
        ...
        if (!mFreeScroll) {
            ...
            /// M: Add for screen cycle for op09.
            ...
            if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                    (isFling && !isVelocityXLeft)) && (mCurrentPage > 0  || isSupportCycleSlidingScreen())) {
                /// M: modify to cycle sliding screen.
                if (mCurrentPage > 0) {
                ...
                if(mFlingState !=FLING_STATE.UP&&mFlingState!=FLING_STATE.DOWN)
                mCurrentPage=finalPage;
            } else if (((isSignificantMove && isDeltaXLeft && !isFling) ||
                    (isFling && isVelocityXLeft)) &&
                    (mCurrentPage < getChildCount() - 1 || isSupportCycleSlidingScreen())) {
                /// M: modify to cycle sliding screen.
              ...
              if(mFlingState !=FLING_STATE.UP&&mFlingState!=FLING_STATE.DOWN)
                ...
                if(mCurrentPage ==getPageCount()-1 && getScrollX()<0){
                	if(getChildCount()<=1) {
	 ...
	 * @return
	 */
	private boolean handleMaskPointMoveEvnet(MotionEvent ev) {
...
//                scrollBy((int) deltaX, 0);
	...
	private void handleMaskPointDwonEvnet(MotionEvent ev) {
		if (ev.getPointerCount()>1) {
			mActivePointerId1 = ev.getPointerId(1);
			if(!mScroller.isFinished()) {
	             mScroller.abortAnimation();//如果没有停止滑动先停止
	         }

	         // Remember where the motion event started
			try {
		         mDownMotionX = mLastMotionX = ev.getX(mActivePointerId1);
		         /// M: [Performance] Scroll page in first performTraversal to improve response time.
		         mLastX = mLastMotionX;
		         mLastY = mLastMotionY;
		         mDownScrollX = getScrollX();
	...
	protected void handleTouchMoveY(MotionEvent ev) {
        ...
        mDownRawY = ev.getRawY();
        final float deltaRawY = mLastRawY-mDownRawY;
        float  f = deltaRawY/500;
//        LogUtils.i("zhouerlong", "deltaRawY::"+deltaRawY+"   f =:"+f);
	}
...
//            LogUtils.i("zhouerlong", "mDownMotionY::"+mDownMotionY);
        ...
        case MotionEvent.ACTION_UP:
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
            	handleMaskPointUpEvent(ev,false);
...
//        ((Launcher) getContext()).onClick(this);
    ...
    int getPageNearestToCenterOfScreen() {
        ...
        float m =0f;
        float ms = 0f;
		if (getScrollX() - getViewportWidth() * mCurrentPage > 0) {
			m = 0.75f;
			ms = 1 - m;
        ...
        int screenCenter = getViewportOffsetX() + getScrollX() + (int)(getViewportWidth() *m);
        final int childCount = getChildCount();
        for (int i = 0; i < childCount; ++i) {
            View layout = (View) getPageAt(i);
            int childWidth = layout.getMeasuredWidth();
            int halfChildWidth =(int) (childWidth *ms);
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        /// M: modify to cycle sliding screen.
        if (isSupportCycleSlidingScreen() && ((mCurrentPage == 0 && whichPage == getChildCount() - 1)
                || (mCurrentPage == getChildCount() - 1 && whichPage == 0))) {
            // When user quick scroll the page like this, pageCount -2 --> pageCount -1 --> 0, there will be a touch down
            // before this call, the scroller is force stopped, the scrollX won't be bigger than mMaxScrollX but the current
            // page is already pageCount -1, so we use half size to restrict this.
			if (getScrollX() < 0
					|| (mMotionEvent != null
							&& mMotionEvent.getPointerCount() <= 1 && mCurrentPage == 0&&getChildCount()>2)) {
				
				mUnboundedScrollX = getChildCount() * getViewportWidth()
						+ getScrollX();// add by zel

			} else {
				if (mMotionEvent != null && mMotionEvent.getPointerCount() > 1||getChildCount()<=2) {
					if (getChildCount()>1&&getScrollX() > mMaxScrollX) {
						mUnboundedScrollX -= mMaxScrollX;
					}
				} else {
					if (mCurrentPage == getChildCount() - 1&&getChildCount()>1)
						mUnboundedScrollX -= mMaxScrollX;
				}
        ...
        if (isSupportCycleSlidingScreen() && ((mCurrentPage == 0 && whichPage == getChildCount() - 1)
            ...
            if (newX == 0 && (getScrollX() <= 0 || getScrollX() >= mMaxScrollX)) {
            	if(getScrollX()<0&&getChildCount()==1) {
                    delta += getViewportWidth();//add by zel
...
//        Log.i("zhouerlong", "whichPage   "+whichPage);
        ...
        if (isSupportCycleSlidingScreen() && ((mCurrentPage == 0 && whichPage == getChildCount() - 1) ||
            ...
            if (getScrollX() <= 0) {
                	...
                	mUnboundedScrollX = getChildCount() * getViewportWidth() + getScrollX();//add by zel
                	if(getChildCount()==2&&!left) {
                		mUnboundedScrollX=0;
                	}
                ...
                mUnboundedScrollX =- getViewportWidth();
            	if(getChildCount()==2&&left) {
                	mUnboundedScrollX =  getViewportWidth();//add by zel
//            		mUnboundedScrollX=0;
            	}
...
//                delta += getViewportWidth();//add by zel
            	if(getScrollX()<0&&getChildCount()==1) {
                    delta += getViewportWidth();//add by zel
                    mUnboundedScrollX = -delta;
            	}else if(getScrollX()>0&&getChildCount()==1){
            		mUnboundedScrollX=getScrollX();
            		delta=-mUnboundedScrollX;
            	}
            }
        }
...
//        Log.i("zhouerlong", "whichPage   "+whichPage);
    ...
    public void snapToPage(int whichPage,boolean immediate) {
    	snapToPageWithLeftFrame(whichPage,mMaxScrollX/getChildCount()-1,PAGE_SNAP_ANIMATION_DURATION,false); 
    }
    protected void snapToPageWithLeftFrame(int whichPage, int delta, int duration, boolean immediate) {
        if (LauncherLog.DEBUG) {
        }
    ...
    public void scrollRight() {
        ...
        if (getNextPage() < getChildCount() -1) {
            snapToPage(getNextPage() + 1);
        } else if (isSupportCycleSlidingScreen() && getNextPage() <= getChildCount()-1) {
            /// M: modify to cycle sliding screen.
            snapToPage(0);
        }
    }
    ...
    protected void invalidatePageData(int currentPage, boolean immediateAndOnly) {
        ...
        if (mContentIsRefreshable) {
            ...
            final int count = getChildCount();
            mDirtyPageContent.clear();//是否已经用过的
            for (int i = 0; i < count; ++i) {
                mDirtyPageContent.add(true);
            }

            // Load any pages that are necessary for the current window of views
            loadAssociatedPages(mCurrentPage, immediateAndOnly);//这个是在syncpage里面刷新数据
            if (LauncherLog.DEBUG) {
            }
    ...
    private PointF isFlingingToDelete() {
        ...
        mVelocityTracker.computeCurrentVelocity(UNITIS, config.getScaledMaximumFlingVelocity());

        if (mVelocityTracker.getYVelocity() > mFlingToShowSystemUIThresholdYVelocity) {//add by zhouerlong
				//add by zhouerlong
	...
	private FLING_STATE isFlingingToShowSysteUI() {
		ViewConfiguration config = ViewConfiguration.get(getContext());
		mVelocityTracker.computeCurrentVelocity(UNITIS,
		...
		float velocitxTracker = mVelocityTracker.getXVelocity();
				//modify by zhouerlong prize 0730 begin
		if (velocityTracker > mFlingToShowSystemUIThresholdYVelocity 
				&& Math.abs(velocitxTracker/velocityTracker) < Math.sin(Math.PI*30/180)
			...
			return FLING_STATE.DOWN;
				...
				&& Math.abs(velocitxTracker/velocityTracker) < Math.sin(Math.PI*45/180)
				/*&& velocitxTracker < mFlingToShowSystemUIThresholdXVelocity
				&& velocitxTracker > -mFlingToShowSystemUIThresholdXVelocity*/) {
        ...
        /*PRIZE-launcher3-zhouerlong-2015-7-23-start*/
	private FLING_STATE multiFingerFlinging(MotionEvent ev) {
		...
		float velocityTracker = mVelocityTracker.getYVelocity();
		float velocitxTracker = mVelocityTracker.getXVelocity();
		float velocitx1Tracker = 0;
		float velocity1Tracker = 0;

		if (ev.getPointerCount() > 1) {
			int point1 = ev.getPointerId(1);
			velocity1Tracker = mVelocityTracker.getYVelocity(point1);
			velocitx1Tracker = mVelocityTracker.getXVelocity(point1);
		}

		if (velocityTracker > mFlingToShowSystemUIThresholdYVelocity) {
			if (ev.getPointerCount() > 1
					&& velocity1Tracker > mFlingMultiFingerYVelocity) {
				return FLING_STATE.MULTI_DOWN;
			}
			if (Math.abs(velocitxTracker) < mFlingToShowSystemUIThresholdXVelocity) {
